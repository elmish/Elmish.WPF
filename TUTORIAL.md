...
through the `args` parameter, but this is immediately passed into the `inherit ViewModelBase<Model, Msg>(args)` line to
...
whole `timerTick` function (without applying `dispatch`) has the signature `Sub<Msg>`:
...
(You don’t have to use `list`; often it will make sense to have `Map<BookId, Book>` to easily and efficiently get a book
by its ID.)
