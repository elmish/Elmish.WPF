...
module Helpers2 =
  ...
  let measure
    ...
    f
    =
    if not <| logPerformance.IsEnabled(LogLevel.Trace) then
      f
    else
      fun a ->
  ...
  let measure2 (logPerformance: ILogger) performanceLogThresholdMs name nameChain callName f =
    if not <| logPerformance.IsEnabled(LogLevel.Trace) then
      ...
      fun a -> measure logPerformance performanceLogThresholdMs name nameChain callName (f a)
...
type Initialize<'t>
  ...
  member _.Base<'model, 'msg>
    ...
    | SubModelSeqKeyedData d ->
      let d =
        d
        |> BindingData.SubModelSeqKeyed.measureFunctions measure measure measure2 measure
  ...
  member _.Base<'model, 'msg>(newModel: 'model, binding: BaseVmBinding<'model, 'msg, 't>) =
    ...
    | SubModelSeqUnkeyed b ->
      ...
      let create m idx =
        ...
        let args =
          ViewModelArgs.create m (fun msg -> d.ToMsg (b.GetCurrentModel()) (idx, msg) |> b.Dispatch) chain loggingArgs
    ...
    | SubModelSeqKeyed b ->
      ...
      let create m id =
        ...
        let args =
          ViewModelArgs.create m (fun msg -> d.ToMsg (b.GetCurrentModel()) (id, msg) |> b.Dispatch) chain loggingArgs
