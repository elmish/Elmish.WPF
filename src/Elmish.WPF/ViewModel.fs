...
and [<AllowNullLiteral>] internal ViewModel<'model, 'msg>
  ...
  let initializeBinding =
    let baseCase (logPerformance: ILogger) (performanceLogThresholdMs: int) (name: string) (nameChain: string) (getFunctionsForSubModelSelectedItem: string -> ((obj -> obj) * (obj -> ViewModel<obj, obj> option)) option) (initialModel: 'model) (getCurrentModel: unit -> 'model) (dispatch: 'msg -> unit) b =
      ...
      | SubModelSelectedItemData d ->
          ...
          |> Option.map (fun (getId, fromId) ->
              ...
              |> BaseVmBinding
              |> (fun b -> b.AddCaching))
    let rec recursiveCase (logPerformance: ILogger) (performanceLogThresholdMs: int) (name: string) (nameChain: string) (getFunctionsForSubModelSelectedItem: string -> ((obj -> obj) * (obj -> ViewModel<obj, obj> option)) option) (initialModel: 'model) (getCurrentModel: unit -> 'model) (dispatch: obj -> unit) b =
      let measure x = x |> Helpers2.measure logPerformance performanceLogThresholdMs name nameChain
      match b with
      | BaseBindingData d -> d |> baseCase logPerformance performanceLogThresholdMs name nameChain getFunctionsForSubModelSelectedItem initialModel getCurrentModel (box >> dispatch)
      | CachingData d ->
          d
          |> recursiveCase logPerformance performanceLogThresholdMs name nameChain getFunctionsForSubModelSelectedItem initialModel getCurrentModel dispatch
      ...
      | WrapDispatchData d ->
          d.BindingData
          |> recursiveCase logPerformance performanceLogThresholdMs name nameChain getFunctionsForSubModelSelectedItem initialModel getCurrentModel (d.WrapDispatch dispatch)
    recursiveCase
  ...
  let (bindings, validationBindings) =
    ...
    for b in sortedBindings do
      if bindingDict.ContainsKey b.Name then
        ...
        initializeBinding logPerformance performanceLogThresholdMs b.Name nameChain getFunctionsForSubModelSelectedItem initialModel (fun () -> currentModel) (unbox >> dispatch) b.Data
