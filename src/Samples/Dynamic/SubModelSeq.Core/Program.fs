...
module Identifiable =
    ...
    type Identifiable<'a> = { Id: Guid; Value: 'a }
...
module Counter =
    ...
    type Counter = { Count: int; StepSize: int }
...
module RoseTree =
    ...
    module RoseTree =
        ...
        let create data children = { Data = data; Children = children }
...
module App =
    ...
    let init () =
        ...
        { SomeGlobalState = false
          DummyRoot = createNewLeaf () |> List.singleton |> RoseTree.create dummyRootData }
...
module Bindings =
    ...
    type SelfWithParent<'a> = { Self: 'a; Parent: 'a }

    let moveUpMsg (_, { Parent = p; Self = s }) =
        match p.Children |> List.tryHead with
        | Some c when c.Data.Id <> s.Data.Id -> OutMoveUp |> Some
        | _ -> None

    let moveDownMsg (_, { Parent = p; Self = s }) =
        match p.Children |> List.tryLast with
        | Some c when c.Data.Id <> s.Data.Id -> OutMoveDown |> Some
        | _ -> None

    let rec subtreeBindings
        ()
        : Binding<
              Model * SelfWithParent<RoseTree<Identifiable<Counter>>>,
              InOutMsg<RoseTreeMsg<Guid, SubtreeMsg>, SubtreeOutMsg>
           > list
        =
        let counterBindings =
            Counter.bindings ()
        ...
        let inMsgBindings =
            [ "CounterIdText" |> Binding.oneWay (fun (_, { Self = s }) -> s.Data.Id)
              ...
              |> Binding.mapModel (fun (m, { Self = p }) ->
                  p.Children |> Seq.map (fun c -> m, { Self = c; Parent = p }))
    ...
    let rootBindings () : Binding<Model, Msg> list =
        [ "Counters"
          ...
          |> Binding.mapModel (fun m ->
              m.DummyRoot.Children |> Seq.map (fun c -> m, { Self = c; Parent = m.DummyRoot }))
          |> Binding.mapMsg (fun (cId, inOutMsg) ->
              match inOutMsg with
              | InMsg msg -> (cId, msg) |> BranchMsg
              | OutMsg msg -> cId |> mapOutMsg msg |> LeafMsg
