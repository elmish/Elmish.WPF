        ...
        try
            (this.Get propName: INotifyCollectionChanged)
                .CollectionChanged.Add(fun e ->
                    ccTriggers.AddOrUpdate(propName, [ e ], (fun _ me -> e :: me)) |> ignore)
        with _ ->
...
module TwoWayValidate =
    ...
    let ``when model is updated, should trigger NotifyPropertyChanged on HasErrors iff the value returned by validate changes``
        ...
        <| property {
            ...
            test
                <@
                    vm.NumPcTriggersFor "HasErrors" = if
                                                          (validate m1 |> ValueOption.isNone) = (validate m2
                                                                                                 |> ValueOption.isNone)
                                                      then
                                                          0
                                                      else
            ...
            | ValueSome sm ->
                test
                    <@
                        ...
                        |> Option.ofObj
                        |> Option.map (fun vm -> vm.CurrentModel) = (m
                                                                     |> getModels
                                                                     |> List.tryFind (fun x -> getId x = getId sm))
                    @>
        }
