    ...
    let ``should return error on bad typing`` () =
        ...
        let vmBinding =
            ...
            |> Option.defaultWith (fun () ->
                failwith $"Could not create VmBinding after passing in BindingData: {binding}")
...
module Update =
    ...
    let check<'a when 'a: equality> (g: Gen<'a>) =
        ...
        <| property {
            ...
            let updateResult =
                Update(LoggingViewModelArgs.none, name).Recursive(initialModel, newModel, vmBinding)
